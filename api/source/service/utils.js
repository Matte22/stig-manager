...
module.exports.selectCollectionByAssetId = async function (assetId) {
  ...
  return _this.pool.query(`SELECT * from enabled_collection where collectionId = (select collectionId from enabled_assets where assetId = ?)`, [assetId])
}
...
module.exports.pruneCollectionRevMap = async function (connection) {
  ...
  left join( select distinct a.collectionId, sa.benchmarkId from stig_asset_map sa left join enabled_assets a using (assetId)) maps using (collectionId, benchmarkId)
