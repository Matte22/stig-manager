...
SM.RuleContentTpl = new Ext.XTemplate(
  ...
  SM.TruncateRecordProperty = function (record, property) {
      ...
      if (value.length > SM.TruncateLimit) {
         return `${value.slice(0,SM.TruncateLimit)}... <span class=sm-truncated-action onclick="SM.ShowUntruncated('${record.store.storeId}','${record.id}','${property}')">Double-click to view full text</span>`
      }
      else {
          return value
      }
